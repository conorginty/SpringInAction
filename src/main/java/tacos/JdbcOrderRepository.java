...
public class JdbcOrderRepository implements OrderRepository {
    ...
    public TacoOrder save(TacoOrder order) {
        ...
        pscf.setReturnGeneratedKeys(true); // needed to later fetch the saved order’s ID

        order.setPlacedAt(new Date());

        PreparedStatementCreator psc =
            pscf.newPreparedStatementCreator( Arrays.asList(
                order.getDeliveryName(), order.getDeliveryStreet(), order.getDeliveryCity(), order.getDeliveryState(),
